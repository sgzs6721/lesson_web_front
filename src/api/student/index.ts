...
const STUDENT_API_PATHS = {
  ...
  CLASS_RECORDS: (studentId: string) => `/api/students/${studentId}/class-records`,
  PAYMENT_RECORDS: (studentId: string) => `/api/students/${studentId}/payment-records`,
};
...
export const student = {
  ...
  getDetail: async (id: string): Promise<Student> => {
    ...
    return request(`${API_HOST}${STUDENT_API_PATHS.DETAIL(id)}`);
  },
  ...
  add: async (data: Omit<Student, 'id'>): Promise<Student> => {
    ...
    return request(`${API_HOST}${STUDENT_API_PATHS.ADD}`, {
      method: 'POST',
  ...
  update: async (id: string, data: Partial<Student>): Promise<Student> => {
    ...
    return request(`${API_HOST}${STUDENT_API_PATHS.UPDATE(id)}`, {
      method: 'PUT',
      body: JSON.stringify(data)
    });
  },
  ...
  delete: async (id: string): Promise<null> => {
    ...
    return request(`${API_HOST}${STUDENT_API_PATHS.DELETE(id)}`, {
      method: 'DELETE'
    });
  },
  ...
  getClassRecords: async (studentId: string): Promise<ClassRecord[]> => {
    ...
    return request(`${API_HOST}${STUDENT_API_PATHS.CLASS_RECORDS(studentId)}`);
  },
  ...
  getPaymentRecords: async (studentId: string): Promise<PaymentRecord[]> => {
    ...
    return request(`${API_HOST}${STUDENT_API_PATHS.PAYMENT_RECORDS(studentId)}`);
  }
};
